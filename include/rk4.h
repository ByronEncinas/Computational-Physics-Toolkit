...
float function(float *matrix1, float *matrix2, int len, int eqs, float *consts){
	...
	float hold[len]; //Hold defines the variables we are working with, that is x_0,y_0, x_1,y_1,..., x_n,y_n
	
	for(i=0;i<=len;i++){
		
		hold[i] = matrix1[i] + matrix2[i]; //This implement the changes required in the RK4 method, matrix1 is the initial value matrix, whereas matrix2
...
//This function will allow us to implement the RK to n number of variables
...
//This function is the core of the RK4 method, it will call all the previous one to accomplish it.
...
float RK(float h,float *matrix1, int eqs,int len, float *consts){
	float k1,k2,k3,k4;
...
//Running the RK algorithm for all the variables in the SDE
		for (k=0;k<=number/2;k++){
				matrix[2*k+1] = matrix[2*k+1]+RK(h, matrix, k,number, consts); 
				matrix[2*k]	+= h;	
		}
